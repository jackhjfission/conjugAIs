...
A Python monorepo for conjugais with automated CI/CD workflows and development container support.
...
- **User**: Non-root user (`aprendiz`) for security
- **SSH Keys**: Automatically mounted from `~/.ssh` on host
- **Workspace**: Mounted at `/home/aprendiz/workspace`
...
```bash
cd conjugais_core  # or conjugais_tools
...
```bash
cd conjugais_core  # Run from package directory
...
```env
IMAGE_NAME=conjugais-dev          # Docker image name
IMAGE_VERSION=v0.0.0        # Docker image version
USER_NAME=aprendiz         # Container user name
```
